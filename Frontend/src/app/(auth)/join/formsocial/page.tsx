...
function JoinSocailPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const userData = {
      snsId: snsId.get('snsId'),
      snsType: snsType.get('snsType'),
      email: email.get('email'),
      name,
      phoneNumber: fullPhoneNumber,
    };

    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/sns-join`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className='w-full h-[948.6px] flex-col justify-center items-center'>
      <form onSubmit={handleSubmit}>
        ...
        <Suspense>
          <div className='w-full h-[218.4px] px-2 flex-col justify-center items-center'>
